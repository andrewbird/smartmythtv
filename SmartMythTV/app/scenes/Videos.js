...
SceneVideos.prototype.initialize = function () {
	$('#svecListbox_BOVI').sfList({itemsPerPage:10});
	$('#svecScrollbar_UKVI').sfScroll({page:0});
...
SceneVideos.prototype.handleKeyDown = function (keyCode) {
	switch (keyCode) {
		...
		case sf.key.DOWN:
			...
			break;
		case sf.key.ENTER:
			Data.currentTitle = Data.Titles[$('#svecListbox_BOVI').sfList('getIndex')];
			...
			sf.scene.show('Player',{parent:"Videos"});
			sf.scene.focus('Player');
			break;
		case 108: //RED
			$('#svecPopup_ok_cancel_0AVI').sfPopup({
				text:'Do you really want to delete '+Data.Titles[$('#svecListbox_BOVI').sfList('getIndex')]+'?', 
				buttons:['Yes', 'No'], 
				callback:function (rlt){
					if(rlt==0) {
						$('#svecLoadingImage_RBMO').sfLoading('show');
						ServiceAPI.deleteRecording(SceneVideos.prototype.getRecording());
